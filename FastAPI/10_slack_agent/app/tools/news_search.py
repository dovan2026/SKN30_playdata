...
async def search_news(
    ...
    *,
    ...
    return await provider.search(query=query, limit=limit, days=days)from __future__ import annotations
...
def _normalize_results(payload: dict[str, Any], limit: int) -> list[NewsItem]:
    """Tavily 응답을 애플리케이션의 NewsItem 목록으로 변환함."""
    items: list[NewsItem] = []
    seen_urls: set[str] = set() # 동일 url이면 여러 번 포함되는 것을 방
    for raw in payload.get("results") or []:
        if not isinstance(raw, dict):
            continue
        title = str(raw.get("title") or "").strip()
        ...
        for attempt in range(1, self._max_attempts + 1):
            try:
                payload = await self._client.search(
                    ...
                    max_results=limit,  # 최대 검색 결과 개수
                    search_depth="basic",   
                    include_answer=False,       # Tavily의 요약 답변 제외
                    include_raw_content=False,  # 원문 전체 내용 제외
                    include_images=False,       # 이미지 제외
                )
                # Tavily 응답을 NewsItem 목록으로 정규화
                items = _normalize_results(payload, limit)
                # 정상적으로 사용할 수 있는 결과가 하나도 없는 경우
                if not items:
                    raise NoNewsResultsError(
                        "검색 결과에서 분석 가능한 출처를 찾지 못했습니다."
                    )
...
async def search_news(
    ...
    *,
    ...
    return await provider.search(query=queㅁㅁry, limit=limit, days=days)
