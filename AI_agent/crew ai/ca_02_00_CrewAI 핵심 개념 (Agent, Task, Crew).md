...
이 실습 문서는 **CrewAI** 라이브러리의 핵심 개념을 초급자도 이해할 수 있도록 안내합니다. 1부에서는 CrewAI의 기본 **구조**(Agent, Task, Crew 등)를 이해하고, 2부에서는 두 에이전트가 **협업**하여 작업을 수행하는 예제를 실습합니다. 각 파트별로 **목표**, **사전 요구사항**, **코드 및 상세 설명**, **실행 방법**, **예상 출력 예시**를 포함하고 있으니 차례대로 따라해 보세요.
...
- `process=Process.sequential`: **프로세스**는 Crew가 태스크를 실행하는 **방식**이나 **전략**을 정의합니다. `Process.sequential`로 지정하면 **태스크들을 순차적으로** 실행합니다. 즉, `tasks` 리스트에 주어진 순서대로 하나씩 차례로 수행합니다. (현재 CrewAI의 주요 프로세스는 순차 실행과 계층적 실행이며, 이 실습에서는 가장 기본인 순차 실행을 사용합니다.)
...
- `process=Process.sequential`: 순차 처리 프로세스를 사용합니다. 첫 번째 태스크부터 차례로 두 번째 태스크까지 진행합니다. (복잡한 위임 구조가 필요하면 `Process.hierarchical`을 사용할 수 있지만, 이 예제에서는 순차가 자연스럽습니다. 조사 후 작성이므로 순서가 중요하기 때문입니다.)
...
- CrewAI는 여기서 다룬 것 외에도 고급 기능들(예: 에이전트 간 **Tool** 사용, **매니저 에이전트 지정**, 계층적 실행 등)을 제공하고 있습니다. 추후 더 복잡한 프로젝트에 도전할 때 이러한 기능들도 학습하면 더욱 강력한 AI 에이전트 팀을 구현할 수 있습니다.
