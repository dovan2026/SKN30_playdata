...
- 예제 시나리오 파일을 실행하며 **도구 사용 흐름**을 단계별로 경험  
- 도구 입력 형식과 관련된 **잠재적 이슈 및 디버깅 방법** 이해  
- 각 시나리오 실행 시 **기대 출력 예시**를 확인하고 결과를 해석  

**사전 요구사항:** 이 실습을 진행하려면 Python과 기본적인 프로그래밍 지식이 필요하며, `crewai[tools]`가 설치되어 있어야 합니다. 또한 `SerperDevTool` 사용을 위해 `SERPER_API_KEY`가 `.env`에 설정되어 있어야 하고, 웹 검색 및 웹 스크래핑을 위해 인터넷 연결이 필요합니다.

## 각 도구의 역할 및 코드 설명

CrewAI에서는 `crewai_tools`가 제공하는 공식 도구를 바로 사용할 수 있고, 필요한 경우 `BaseTool`을 상속해 직접 도구를 만들 수 있습니다. 에이전트(LLM)는 도구의 `name`과 `description` 정보를 바탕으로 어떤 도구를 언제 사용할지 판단하며, 도구 실행 결과를 받아 Task를 해결합니다. 


### SerperDevTool (웹 검색 도구)

`SerperDevTool`은 **웹 검색을 수행**하는 CrewAI 공식 도구입니다. Google Search API 기반의 Serper 서비스를 사용하므로 `SERPER_API_KEY` 환경변수가 필요합니다. 이 도구를 사용하면 에이전트가 인터넷에서 최신 자료를 검색할 수 있습니다.
...
- `SerperDevTool()` 인스턴스를 생성하면 에이전트의 `tools` 목록에 바로 전달할 수 있습니다.

- 에이전트가 “최신 AI 뉴스”를 검색해야 한다고 판단하면 `SerperDevTool`을 호출하고, 검색 결과 텍스트를 바탕으로 다음 추론을 이어갑니다.

### ScrapeWebsiteTool (웹 스크래퍼 도구)

`ScrapeWebsiteTool`은 **웹 페이지의 본문 내용을 추출**하는 CrewAI 공식 도구입니다. 에이전트가 특정 URL을 주거나, 도구 생성 시 `website_url`을 지정하면 해당 페이지를 가져와 텍스트 내용을 반환합니다. 이를 통해 에이전트는 **검색으로 찾은 웹 페이지의 상세 정보**를 읽을 수 있습니다.
...
- `ScrapeWebsiteTool()`은 실행 시점에 URL을 받을 수 있고, `ScrapeWebsiteTool(website_url="...")`처럼 특정 사이트를 대상으로 고정해 둘 수도 있습니다.

- 이 도구를 통해 에이전트는 검색 단계에서 얻은 링크의 실제 내용을 읽고 요약하거나 분석할 수 있게 됩니다.
...
- `CalculatorTool` 클래스 정의와 이름, 설명 설정입니다. 이름은 "계산기", 설명은 "수학 계산을 수행합니다."로 정의되어 있어 **수학적 표현식을 계산해주는 도구**임을 알 수 있습니다.

- `_run(self, expression: str)` 메서드 구현 부분입니다. `expression`이라는 문자열로 수학 표현식을 받아 평가한 결과를 반환합니다.
...
**사전 준비 확인:** 
- `.env` 파일에 필요한 API 키 (예: OpenAI 키)가 설정되어 있는지 확인하세요.
- 인터넷 연결이 되어 있어야 Serper 웹 검색과 웹 스크래퍼가 동작합니다.
- 각각의 `scenario` 파일이 같은 폴더에 있고 `crewai` 라이브러리를 잘 import할 수 있는 상태여야 합니다.
...
먼저 `04-1.scenario_news.py` 파일을 열어보면, **“뉴스 분석가” 에이전트**가 정의되어 있습니다. 이 에이전트는 **“최신 뉴스를 검색하여 요약”** 하는 목표를 가지고 있으며, `SerperDevTool`과 `ScrapeWebsiteTool` 두 가지 도구를 사용하도록 설정되어 있습니다.
...
- **에이전트 설정:** `Agent(...)`를 생성할 때 역할(`role`), 목표(`goal`), 백스토리(`backstory`), 사용 도구들(`tools`), LLM 모델(`llm`), 그리고 `verbose=True` 등을 설정합니다. 
  ...
  - `tools=[search_tool, scrape_tool]` 부분을 통해 앞서 생성한 두 도구를 에이전트에게 장착시킵니다. 이로써 에이전트는 **Serper 웹 검색과 웹 스크래핑 기능을 활용**할 수 있게 됩니다.
...
실행하면 콘솔에 `verbose=True`에 의해 **에이전트의 사고 과정과 도구 사용 단계**가 출력됩니다. 예를 들어, **SerperDevTool을 사용**하여 웹 검색을 하고, **ScrapeWebsiteTool을 사용**하여 각 뉴스 링크의 내용을 읽어오는 순서 등이 로그로 나타날 것입니다. 

에이전트는 아마 다음과 같은 추론 과정을 거칠 것입니다 (개략적인 예시):

1. "최근 인공지능 관련 주요 뉴스 3건이 무엇일까?"라고 스스로 생각한 뒤, **Serper 검색 도구**를 이용해 관련 키워드로 검색을 시도합니다.  
2. SerperDevTool이 반환한 검색 결과 문자열을 보고, 그 중 적절한 뉴스 기사 링크들을 **파악하여 3개 선택**합니다. (에이전트 LLM이 검색 결과 텍스트에서 URL을 찾아낼 것입니다.)  
3. 선택된 각 링크에 대해 **ScrapeWebsiteTool**을 호출하여 기사 본문 내용을 가져옵니다.  
...
다음은 `04-2.scenario_recipe.py` 파일을 실행해 보겠습니다. 이 시나리오에서는 **“요리 전문가” 에이전트**가 **채식 파스타 레시피를 추천하고 재료와 조리법을 안내**해줍니다. 사용되는 도구는 **검색(`SerperDevTool`)과 스크래퍼(`ScrapeWebsiteTool`)**로, 뉴스 시나리오와 동일하게 두 가지 도구를 활용합니다 (계산기는 사용하지 않습니다).
...
1. 에이전트가 SerperDevTool을 사용하여 "채식 파스타 레시피" 등의 키워드로 **웹 검색**을 합니다.
2. 검색 결과에서 인기 있는 **채식 파스타 레시피 웹페이지 링크**를 찾아냅니다 (예를 들면 채식 볼로네제 파스타 블로그 글 등).
3. 해당 링크에 대해 ScrapeWebsiteTool을 사용하여 **레시피 상세 내용을 수집**합니다.
...
이제 세 번째로 `04-3.scenario_travel.py` 파일을 살펴보고 실행해보겠습니다. 이 시나리오는 **“여행 전문가” 에이전트**를 이용하여 **파리 5일 여행 일정과 예산 계획**을 세우는 예제입니다. 앞선 두 시나리오와 달리, 여기서는 **세 가지 도구**가 사용됩니다: **검색**, **스크래퍼**, **계산기**.
...
2. 검색 결과에서 **유용한 일정 정보**나 **맛집/문화 장소 추천** 관련 링크를 찾아 **ScrapeWebsiteTool**로 내용을 확인합니다. (예를 들어, 여행 블로그의 파리 여행기나 관광청 사이트의 정보를 읽을 수 있습니다.)
...
[뉴스 시나리오 응답]
 1. **배경훈 부총리, 인공지능안전연구소 방문…”AI 안전 정책 방향 논의**  
   배경훈 부총리 겸 과학기술정보통신부장관이 인공지능안전연구소를 방문하여 AI 안전 정책 방향에 대해 논의했습니다. 부총리는 딥페이크 탐지 기술과 AI의 안전성을 확보하기 위한 연구 평가를 검토했습니다.  
   [Link to article](https://www.aitimes.com/news/articleView.html?idxno=17337)

2. **오픈AI “민간 협력으로 데이터센터 구축하는 것이 국내 핵심 전략**  
   오픈AI가 한국에서의 전략으로 데이터센터 구축을 위해 삼성, SK와 협력하는 내용을 발표했습니다. 이 계약에 따라 한국 내 AI 데이터센터 개발이 추진될 예정입니다.  
   [Link to article](https://www.aitimes.com/news/articleView.html?idxno=17338)

3. **엔비디아 'H100', 다음 달 우주 간다...크루소 “세계 첫 우주 클라우드 가동”**  
   엔비디아의 H100 GPU가 궤도에 올려질 위성 데이터센터에 탑재됩니다. 이는 우주에서 운영될 최초의 퍼블릭 클라우드를 목표로 하며, 많은 AI 용도로 활용될 예정입니다.  
   [Link to article](https://www.aitimes.com/news/articleView.html?idxno=17339)
...
- **도구 구현 상세:** 공식 `crewai_tools`의 `SerperDevTool`(웹 검색), `ScrapeWebsiteTool`(웹 콘텐츠 추출), 그리고 직접 정의한 `CalculatorTool`(수학 계산)의 **동작 원리와 사용법**을 이해했습니다. 특히 최신 정보 검색, 웹페이지 내용 추출, 파이썬 `eval`을 제한된 환경에서 사용하는 계산 도구 구현 방법을 접하였습니다.
